...
bool in_shadow(const point3& point, int shapeIndex) {
    for (int j = 0; j < sce.getLights().size(); ++j) {
        ...
        auto lightDistance = (currentLight->getPosition() - point).length();
...
color ray_color(const ray& r, int recursionDepth = 0) {
    shape* closestShape;
    ...
    if (renderMode == "binary") {
        ...
        if (closestShape->get_material().get_is_reflective()) {
            if (recursionDepth <= maxNrBounces) {
                ...
                ray reflectedRay(pointOnSurface, reflectedVector);
